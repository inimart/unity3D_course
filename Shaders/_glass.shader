		...
		Pass
		{
			...
			half4 frag(vertexOutput i): COLOR
			{
				...
				half4 normalColor = tex2D(_NormalMap, i.normalTexCoord.xy);
					...
					TSNormal = normalFromColor(normalColor);
				#else
					TSNormal = normalWorldAtPixel;
				#endif
				...
				//4
				//ComputeGrabScreenPos() will just transform input from clip coordinate vertex position [-w,w] into [0,w
				// then calling tex2Dproj() will transform [0,w] into [0,1], which is a valid texture sampling value
				half4 grabColor = tex2Dproj(_BackgroundTexture, i.texcoordGrab);
