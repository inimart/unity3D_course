	...
	Subshader
	{
		...
		Pass
		{
			...
			struct vertexOutput
			{
				...
				float4 texcoord : TEXCOORD0;
				//5 add +1 to all the TEXCOORD interpolators, because of customData
			...
			vertexOutput vert(vertexInput v)
			{
				...
				//5 *Before* to change texcoord values: xyz RGB customdata2, w CustomData1
                o.customData.xyzw = float4(v.texcoord.w, v.customData.xy, v.texcoord.z);
			...
			half4 frag(vertexOutput i): COLOR
			{
				half4 albedoColor = tex2D(_MainTexture, i.texcoord);
					...
					//7
					float4 emissive;
					emissive.rgb = lerp (i.customData.xyz, float3 (0,0,0), finalAlpha);
